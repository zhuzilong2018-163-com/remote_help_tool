...
//全局变量
uint32 g_beneficiary_sessionId;          //用于通信对话识别
uint8  g_beneficiary_uc_mode_change = 0; //g_uc_mode_change默认为0（为交流模式），1（协助模式）
sem_t  g_beneficiary_sem;                //保证线程收到命令再发送

uint8 buf[5096];



void beneficiary_pthread_message_come(void *argv)
{
...
void beneficiary_pthread_message_outs(void *argv)
{
    ...
    while (1) {
        ...
        sem_wait(&g_beneficiary_sem);  //相当于P操作,数据-1
    }
}

void beneficiary_handler(int tmp)
{
    if (g_beneficiary_uc_mode_change == 0) {
        g_beneficiary_uc_mode_change = 1;
    } else if (g_beneficiary_uc_mode_change == 1) {
        g_beneficiary_uc_mode_change = 0;
    }
}


int32 beneficiary_pthread_communicate_encap_message(char *buff, uint8 message_type)
{
	...
	sem_post(&g_beneficiary_sem);  //相当于V操作,数据+1

    return SUCCESS;
}


void beneficiary_pthread_communicate(uint32 *argv)
{
    char  ac_read_buff[1024];
    char* c_read_buff = ac_read_buff;

    signal(SIGQUIT, beneficiary_handler);
    ...
    while (1) {
        ...
        if (g_beneficiary_uc_mode_change == E_COMMUNICATE) {
            if (beneficiary_pthread_communicate_encap_message(c_read_buff, E_COMMUNICATE) != SUCCESS) {
                ERROR_LOG(HELPER, "%s, type[%d]\n", "beneficiary_pthread_communicate_encap_message error\n", E_COMMUNICATE);
            } 

        } else if (g_beneficiary_uc_mode_change == E_CMD_MODE) {
            if (beneficiary_pthread_communicate_encap_message(c_read_buff, E_CMD_MODE) != SUCCESS) {
                ERROR_LOG(HELPER, "%s, type[%d]\n", "beneficiary_pthread_communicate_encap_message error\n", E_CMD);
            } 
        }
    }
...
void beneficiary_thread_create(void)
{   
	...
	sem_init(&g_beneficiary_sem, 0, 0); //保证命令和交流信息的一收一发
    ...
    //创建消息接收进程             参数需要
    if (pthread_create(&message_come, NULL, (void *)beneficiary_pthread_message_come, (void *)&st_come_pthread_argcv) != 0) {
        ERROR_LOG(HELPER, "%s\n", "beneficiary_pthread_create message_come error\n");
        exit(1);
    } else {
        sleep(300);
    ...
    //创建消息发送进程
    if (pthread_create(&messge_out, NULL, (void *)beneficiary_pthread_message_outs, (void *)&st_out_pthread_argcv) != 0) {
        ERROR_LOG(HELPER, "%s\n", "beneficiary_pthread_create messge_out error\n");  
        exit(1);
    } else {
        sleep(300);
    ...
    //创建消息交互进程
    if (pthread_create(&communicate, NULL, (void *)beneficiary_pthread_communicate, (void *)&st_communicate_pthread_argcv) != 0) {
        ERROR_LOG(HELPER, "%s\n", "beneficiary_pthread_create communicate error\n");
        exit(1);
    } else {
        sleep(300);
    ...
    sem_destroy(&g_beneficiary_sem);
}



int32 beneficiary_auth_request_encap( AUTH_HEADER_REQUEST  *auth_header_request, uint32 ul_auth_request_len,uint8 *uc_message_buff)
{
    //认证消息填充与发送
...
int32 beneficiary_mode_start(void)
{
    int32 l_res = -1;
	int32 sockfd, size;    
    ...
    l_res = beneficiary_auth_request_encap(auth_header_request, ul_auth_request_len, uc_message_buff);
    if (l_res != SUCCESS) {
    ...
    if (auth_header_response->result == 1) {
        g_beneficiary_sessionId = auth_header_response->sessionId;
        INFO_LOG(HELPER, "%s\n", "auth success!!!\n");
    } else {
        ERROR_LOG(HELPER, "%s\n", "auth fail!!!\n");
    }
